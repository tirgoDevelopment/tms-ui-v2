...
export class DriverFormComponent implements OnInit {
  ...
  getById() {
    if (this.id && (this.mode == "view" || this.mode == "edit")) {
      ...
      this.driversService.getById(this.id).subscribe(
  ...
  onPhoneNumberChange(inputValue: string): void {
    ...
    this.debounceTimeout = setTimeout(() => {
      ...
      const matchedCountry = this.countries.find(
        (country) => country.code === inputCode
      );
  ...
  patchForm() {
    ...
    let mainPhoneNumber = this.data.phoneNumbers?.find((phone) => phone.isMain);
    if (!mainPhoneNumber && this.data.phoneNumbers?.length) {
      mainPhoneNumber = this.data.phoneNumbers[0];
    }
    if (mainPhoneNumber) {
      let code = mainPhoneNumber.code?.replace("+", "") || "";
      let number = mainPhoneNumber.number || "";

      if (code === "99" && number.startsWith("8")) {
        code = "998";
        number = number.substring(1); 
      }

      if (code === "998" && number.startsWith("8")) {
        number = number.substring(1);
      }
  ...
  private extractPhoneNumber(
    ...
    const codeMappings: {
      [key: string]: { dialCode: string; sliceIndex: number };
    } = {
    ...
    const numberWithoutCode = phoneNumber
      .replace(/[^0-9]/g, "")
  ...
  sendSms() {
    ...
    const extracted = this.extractPhoneNumber(
      phoneNumbers,
      selectedCountryCode
    );
    if (!extracted) {
        ...
        if (res && res.success) {
          if (res.data.isRegistered) {
            this.toastr.error(
              ...
              "Пожалуйста, отправьте запрос на водтитель."
            );
          } else {
  ...
  sendVerifyedCode() {
    ...
    if (this.isCodeExpired) {
      this.loading = false;
      this.toastr.error(
        "Срок действия SMS-кода истек. Пожалуйста, запросите новый.",
        ""
      );
  ...
  onSubmit() {
    ...
    if (
      this.selectedCountry.code === "+998" ||
      this.selectedCountry.code === "+992" ||
      this.selectedCountry.code === "+996"
    ) {
      phoneNumbers.push({
    ...
    if (this.selectedFilePassport) {
      const file = new File(
        [this.selectedFilePassport],
        Date.now() + this.selectedFilePassport.name,
        { type: this.selectedFilePassport.type }
      );
      formData.append("passport", file);
    }
    if (this.selectedFileLicense) {
      const file = new File(
        [this.selectedFileLicense],
        Date.now() + this.selectedFileLicense.name,
        { type: this.selectedFileLicense.type }
      );
  ...
  onBlock() {
    if (this.data.blocked) {
      this.driversService
        .unblock(this.data.id)
        .subscribe((res: Response<DriverModel>) => {
          this.toastr.success(
            this.translate.instant("successfullyActivated"),
            ""
          );
    ...
    this.confirmModal = this.modal.confirm({
      ...
      nzOnOk: () => {
        this.driversService.block(this.data.id).subscribe((res: any) => {
          if (res?.success) {
            this.toastr.success(
              this.translate.instant("successfullBlocked"),
              ""
            );
            this.drawerRef.close({ success: true });
          }
  ...
  onSendRequest() {
    this.loading = true;
    this.driversService
      .sendRequestToAddDriver({
        driverId: this.selectedDriver,
        tmsId: this.currentUser.merchantId,
      })
      .subscribe(
        (res: any) => {
          this.loading = false;
          if (res && res?.success) {
            this.toastr.success(
              this.translate.instant("successfullSendRequest"),
              this.translate.instant("wait_for_response")
            );
            this.drawerRef.close({ success: true });
          }
        },
